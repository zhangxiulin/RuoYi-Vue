...
import com.ruoyi.integrator.thread.DtxAtomikosAggregateThread;
...
/**
 * @description: JTA/XA Atomikos聚合转发-具体策略角色
 * @author: zhangxiulin
...
public class InDtxAtomikosAggregateStrategy implements IInAggregateStrategy {
    ...
    public DistributedTxSolution getType() {
        return DistributedTxSolution.Atomikos;
    }

    @Override
    public Object aggregate(InAggregateRequestVo request) {
        log.info("聚合服务[" + request.getReqId() + "] 分布式事务策略[Atomikos]");
        AjaxResult ajaxResult = null;
        // 同步、异步
        InAggregation inAggregation = request.getInAggregation();
        if (inAggregation != null){
            DtxAtomikosAggregateThread daat = new DtxAtomikosAggregateThread(inAggregation, request.getVarList(), request.getDataList());
            if (Constants.YES.equals(inAggregation.getIsAsync())) {
                threadPoolTaskExecutor.submit(daat);
                ajaxResult = AjaxResult.success("聚合服务["+request.getReqId()+"]提交成功");
            } else {
                try {
                    ajaxResult = daat.call();
                } catch (Exception e) {
